...
def atlassian_callback():
    ...
    data = response.json()
    print("Atlassian Authentication Response:", data)  # Debug the response
    ...
    print("Atlassian Accessible Resources:", resources)  # Debug accessible resources

    # Assume the first resource's cloud_id for simplicity
    cloud_id = resources[0]["id"] if resources else None

    # Log the requester ID
    print(f"Atlassian Requester ID: {requester if requester else 'Not provided'}")
    print(f"Atlassian Cloud ID: {cloud_id}")
